    ...
    function renderQuestion() {
      if (!filtered.length) {
        container.innerHTML = '<div style="text-align:center;padding:60px 20px;">' +
          '<p style="color:var(--c-ink-muted,#1A1208);font-size:1rem;">No questions match your filters.</p></div>';
        return;
      }
      ...
      container.innerHTML = [
        ...
        /* Nav buttons */
        '<div style="display:flex;gap:10px;justify-content:space-between;align-items:center;flex-wrap:wrap;margin-top:0.5rem;">',
        '  <button onclick="window._neetPrev()" ' + (currentIndex <= 0 ? 'disabled style="opacity:0.4;cursor:not-allowed;"' : '') + ' style="background:var(--opt-bg,#fff);border:1.5px solid var(--c-border,#E8DDD0);color:var(--c-ink-muted,#6B5C45);padding:9px 20px;border-radius:100px;font-size:0.85rem;font-weight:600;cursor:pointer;font-family:inherit;transition:all 0.15s;display:inline-flex;align-items:center;gap:5px;" onmouseover="if(!this.disabled){this.style.borderColor=\'#FF6B1A\';this.style.color=\'#FF6B1A\';}" onmouseout="this.style.borderColor=\'#E8DDD0\';this.style.color=\'#6B5C45\';">← Prev</button>',
        '  <button onclick="window._neetSkip()" style="background:#FF6B1A;border:none;color:#fff;padding:10px 24px;border-radius:100px;font-size:0.875rem;font-weight:700;cursor:pointer;font-family:inherit;box-shadow:0 4px 14px rgba(255,107,26,0.3);transition:all 0.15s;display:inline-flex;align-items:center;gap:5px;" onmouseover="this.style.background=\'#E85500\';this.style.transform=\'translateY(-1px)\';" onmouseout="this.style.background=\'#FF6B1A\';this.style.transform=\'none\';">Skip →</button>',
        '</div>'
      ].join('\n');
